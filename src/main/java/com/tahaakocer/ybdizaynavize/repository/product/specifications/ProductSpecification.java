...
public class ProductSpecification {
    public static Specification<Product> hasAttributeValue(List<String> attributeValues) {
        return (root, query, criteriaBuilder) -> {
            Join<Product, AttributeValue> attributeValueJoin = root.join("variants").join("attributeValues");
            Predicate predicate = attributeValueJoin.get("attributeValue").in(attributeValues);
